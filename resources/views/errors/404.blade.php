...
@section('content')
    <div class="flex flex-col items-center justify-center text-center py-20 sm:py-28">
        ...
        <!-- Icon -->
        <div class="w-20 h-20 flex items-center justify-center rounded-full bg-yellow-100 mb-6">
            <x-lucide-alert-triangle class="w-10 h-10 text-yellow-600" />
        </div>

        <!-- Title -->
        <h1 class="text-4xl font-bold text-gray-900 mb-2">
            {{ __('app/error.404.heading') }}
        </h1>

        <p class="text-gray-600 text-base max-w-md mx-auto mb-8 leading-relaxed">
            {!! __('app/error.404.description') !!}
        </p>

        <!-- Actions -->
        <div class="flex flex-col sm:flex-row items-center justify-center gap-10">
            <button onclick="history.length > 1 ? history.back() : window.location='{{ route('home') }}'"
