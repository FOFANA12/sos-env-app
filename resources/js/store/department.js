...
export const useDepartmentStore = defineStore('department', () => {
  ...
  const getAll = async ({
    ...
    const { data } = await api.get(endpoints.department.getAll, { params });
  ...
  const find = async (id, mode = 'edit') => {
    try {
      const response = await api.get(endpoints.department.find(id, mode));
  ...
  const create = async () => {
    ...
    try {
      const response = await api.post(endpoints.department.create, form.value.data());
      return response.data;
    } catch (error) {
  ...
  const destroy = async (ids) => {
    try {
      const response = await api.post(endpoints.department.destroy, { ids });
      return response.data;
    } catch (error) {
      if (error?.response?.data) throw error.response.data;
  ...
  const requirements = async () => {
    regions.value = [];
    try {
      const { data } = await api.get(endpoints.department.requirements);
